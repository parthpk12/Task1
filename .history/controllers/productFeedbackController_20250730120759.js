...
const getAllFeedBacksByUserId = async (req,res) =>{
    try{
     ...
     const {rows:allFB} = await db.query("select u.username,u.email,p.name,p.price,pf.feedback from product_feedback pf join users u  on pf.user_id  = u.id join products p on pf.product_id = p.id where user_id = $1",[UserId]);
...
const deleteFeedbackByUserIdProductId = () => {
    const data = req.params;
    console.log(data);


    res.json({
     message : "Delete success by UserI"
    })
}
...
module.exports = {GetAllFeedbacks , getAllFeedBacksByProductId , getAllFeedBacksByUserId , deleteFeedbackByUserIdProductId};