...
const getAllFeedBacksByUserId = async (req,res) =>{
    try{
     ...
     const {} = await db.query("select u.username,u.email,p.name,p.price,pf.feedback from feedback fb join users on fb.user_id  = u.id join products on fb.product_id = p.id where user_id = $1",[id]);
