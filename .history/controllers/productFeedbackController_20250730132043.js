...
const deleteFeedbackByUserIdProductId = async (req, res) => {
  try {
    ...
    console.error("Error" + err.message);
    res.send("Error for Delete Feedback by UserId and ProductId");
  }
};

const deleteFeedbackByUserId = async (req,res) => {
    try{

     const {uid} = req.params;

     await db.query("delete from product_feedback where user_id = $1",[uid]);

      res.json({
        message: "Delete success by UserId",
      });

    }catch(err){
       console.log("Error:"+err.message);
       res.send("Error for Delete Feedback by UserId ");
    }
}

const deleteFeedbackByProductId = async (req, res) => {
  try {
     const {pid} = req.params;

     await db.query("delete from product_feedback where product_id = $1",[pid]);

      res.json({
        message: "Delete success by ProductId",
      });

  } 
  catch(err){
    console.log("Error:" + err.message);
    res.send("Error for Delete Feedback by ProductId ");
  }
};
...
const UpdateFeedbackByUserIdProductId = async (req,res) => {
      try{
          ...
          await db.query("update product_feedback set feedback = $1 where user_id = $2 and product_id = $3",[feedback,id,pid]);
